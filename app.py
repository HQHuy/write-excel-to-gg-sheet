    ...
    if request.method == "POST":
        ...
        try:
            ...
            for shop in shop_list:
                ...
                # Mặc định 0 nếu không tìm thấy shop
                non_vat = non_vat_data.get(shop, 0)
                # Mặc định 0 nếu không tìm thấy shop
                vat = vat_data.get(shop, 0)
                # Tạo một hàng mới cho mỗi shop
                raw_data.append([shop, non_vat, vat])

            # Kiểm tra dữ liệu thô
            # print("Raw Data trước khi làm sạch:")
            raw_data = [[shop if pd.notna(shop) else "Unknown", non_vat, vat]
                        for shop, non_vat, vat in raw_data]
            # print(raw_data)
            ...
            result_df = pd.DataFrame(result_data, columns=[
                                     'Shop', 'Doanh thu thực (Non-VAT)', 'Doanh thu (VAT)'])
            ...
            sheet_name = pd.to_datetime(
                date, format="%d/%m/%Y").strftime("%m-%y")
            ...
            if func.check_existing_data(sheets_service, sheet_id, sheet_name, date):
                print("shop_list_dict JSON:", json.dumps(
                    shop_list_dict, indent=4))
